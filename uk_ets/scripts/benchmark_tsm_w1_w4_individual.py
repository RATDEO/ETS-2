#!/usr/bin/env python3
"""Benchmark shared vs individual DLinear on difficult windows W1-W4."""
...
def _write_summary(out_dir: Path, df: pd.DataFrame) -> None:
    ...
    baseline = mean_df[mean_df["candidate"] == "shared_seq20_reg"].iloc[0]
    best = mean_df.iloc[0]
    ...
    lines = [
        ...
        f"- Shared baseline path MSE: `{baseline['path_mse']:.6f}`.",
        f"- Improvement vs shared baseline: `{gain:.2%}`.",
    ]
...
def main() -> int:
    out_dir = (
        PROJECT_ROOT
        / "reports"
        / "uk_ets_tsm_w1_w4_individual_benchmark"
            ...
            rows.append(
                {
                    ...
                    "test_end": window.test_end,
                    "run_dir": str(run_dir),
                    "dlinear_individual": bool(overrides["model"]["dlinear_individual"]),
                    **metrics,
                }
            )
