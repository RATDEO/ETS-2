#!/usr/bin/env python3
"""Benchmark wider effective retrieval on W1-W4 using the improved Huber energy base."""
...
def _candidates() -> list[Candidate]:
    ...
    return [
        Candidate(
            name="effective_retrieval_wide6",
            objective="Widen the actual matched-example set and support-memory budget so the refiner can see more than three matched cases.",
            expectation="Expected mean uplift vs current heuristic: roughly flat to +0.5%. If example starvation is the bottleneck, this should help W3 without blowing up W4.",
            overrides=_merge(
                baseline,
                {
                    "llm": {
                        "cot_rf": {
                            "k_examples": 8,
                            "n_similarity_examples": 4,
                            "online_memory_policy": {
                                "support_examples": 6,
                                "positive_examples": 2,
                                "negative_examples": 1,
                                "max_total_examples": 9,
                            },
                        }
                    },
                    "hdelta": {
        ...
        Candidate(
            name="effective_retrieval_wide8",
            objective="Push the effective retrieval width further with more matched examples and a larger online-memory budget.",
            expectation="Expected mean uplift vs current heuristic: -0.3% to +0.6%. Higher upside if example diversity matters, but also the clearest risk of prompt dilution.",
            overrides=_merge(
                baseline,
                {
                    "llm": {
