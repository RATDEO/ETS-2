#!/usr/bin/env python3
"""Benchmark live-policy shortlist on W1-W4 using the improved Huber energy-interaction base."""
...
def _selected_candidates() -> dict[str, dict[str, Any]]:
    candidate_map = {candidate.name: candidate.overrides for candidate in live_policy_candidates()}
    return {
        "baseline_regime_specific": candidate_map["baseline_regime_specific"],
        "learned_logistic_regime": candidate_map["learned_logistic_regime"],
    }


def _run_candidate(window: WindowSpec, candidate_name: str, candidate_overrides: dict[str, Any]) -> Path:
    overrides = _merge(
        _merge(
            _improved_base_overrides(),
            candidate_overrides,
        ),
        {
            "split": {
...
def _import_gbdt_rows() -> list[dict[str, Any]]:
    df = pd.read_csv(GBDT_IMPORTED_RESULTS)
    df["candidate"] = "learned_gbdt_regime"
...
def main() -> int:
    ...
    if results_path.exists():
        ...
        rows = []
        rows.extend(_import_gbdt_rows())
        pd.DataFrame(rows).to_csv(results_path, index=False)
    completed = {(str(row["candidate"]), str(row["window"])) for row in rows}

    for candidate_name, candidate_overrides in _selected_candidates().items():
        for window in WINDOWS:
            ...
            run_dir = _run_candidate(window, candidate_name, candidate_overrides)
