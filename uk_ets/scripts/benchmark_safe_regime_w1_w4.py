#!/usr/bin/env python3
"""Benchmark safe-regime gate variants on W1-W4 using the regularized shared DLinear base."""
...
def _candidate_overrides() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "safe_h20_2p5_h30_4p0_vol_3p8": _merge(
            gate_root,
            {
                "llm": {
                    "cot_rf": {
...
def _write_summary(out_dir: Path, df: pd.DataFrame) -> None:
    ...
    lines = [
        "# W1-W4 Safe-Regime Benchmark",
        "",
        f"Generated: {datetime.now().isoformat()}",
        "",
        "Frozen base and gate:",
        "- UK data root: `uk_ets/Data_auto_uk`",
        "- shared DLinear regularized: `lr=0.001`, `max_epochs=40`, `patience=8`, `dropout=0.5`, `weight_decay=0.02`",
        "- learned gate schema: `top20`",
        "",
        "The `gate_top20` rows are imported from the completed hard-regime benchmark and serve as the baseline.",
        "",
        "## Results",
        "",
