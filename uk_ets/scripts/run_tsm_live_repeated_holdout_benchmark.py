#!/usr/bin/env python3
"""Run repeated same-geometry production holdouts for the live TSM+LLM stack."""
...
def _run_window(window: HoldoutWindow) -> Path:
    if window.idx == 0 and ANCHOR_RUN_DIR.exists():
        return ANCHOR_RUN_DIR.resolve()
    ...
    run_dir = run_experiment(config_path=CONFIG_PATH, overrides=overrides)
...
def _read_horizon_metrics(run_dir: Path) -> pd.DataFrame:
    df = pd.read_csv(run_dir / "results" / "metrics_by_horizon.csv")
    return df
...
def _write_summary(out_dir: Path, df: pd.DataFrame) -> None:
    ...
    post_cutoff = df[df["llm_cutoff_status"] == "post_cutoff_only"].copy()
    if not post_cutoff.empty:
        lines.extend(
            [
                "",
                "## Post-2025 Windows",
                f"- Windows: `{len(post_cutoff)}`.",
                f"- Mean path-MSE improvement: `{post_cutoff['path_improvement_pct'].mean():.2%}`.",
                f"- Mean `h20` improvement: `{post_cutoff['h20_improvement_pct'].mean():.2%}`.",
                f"- Mean `h30` improvement: `{post_cutoff['h30_improvement_pct'].mean():.2%}`.",
            ]
        )
...
def main() -> int:
    out_dir = (
        PROJECT_ROOT
        / "reports"
        / "uk_ets_repeated_production_holdout"
