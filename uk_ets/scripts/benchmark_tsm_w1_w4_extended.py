#!/usr/bin/env python3
"""Extended TSM-only benchmark on difficult repeated-holdout windows W1-W4."""
...
def _compact_live_features() -> dict[str, Any]:
    out = dict(LIVE_CFG["features"])
    out["max_exogenous_features_model"] = 10
    out["preferred_feature_order"] = out["preferred_feature_order"][:10]
    return out


def _candidate_overrides() -> dict[str, dict[str, Any]]:
    tuned_model = dict(TUNED_CFG["model"])
    live_features = dict(LIVE_CFG["features"])
    tuned_features = dict(TUNED_CFG["features"])
    compact_live_features = _compact_live_features()

    tuned_short_model = dict(tuned_model)
    tuned_short_model.update({"learning_rate": 0.001, "max_epochs": 40, "early_stopping_patience": 8})

    return {
        "live_auto_uk": {
            "llm": {"methods": []},
        },
        "live_short_ret_s10_l5": {
            "llm": {"methods": []},
            "target": {"mode": "returns"},
            "time_series": {"seq_len": 10, "label_len": 5},
            "model": tuned_short_model,
            "features": live_features,
        },
        "live_short_price_s10_l5": {
            "llm": {"methods": []},
            "target": {"mode": "price"},
            "time_series": {"seq_len": 10, "label_len": 5},
            "model": tuned_short_model,
        ...
        "compact_short_ret_s10_l5": {
            "llm": {"methods": []},
            "target": {"mode": "returns"},
            "time_series": {"seq_len": 10, "label_len": 5},
            "model": tuned_short_model,
            "features": compact_live_features,
        },
        "tuned_feature_short_ret_s10_l5": {
            "llm": {"methods": []},
            "target": {"mode": "returns"},
            "time_series": {"seq_len": 10, "label_len": 5},
            "model": tuned_short_model,
...
def main() -> int:
    out_dir = (
        PROJECT_ROOT
        / "reports"
        / "uk_ets_tsm_w1_w4_extended_benchmark"
