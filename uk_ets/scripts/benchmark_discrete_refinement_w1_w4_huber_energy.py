#!/usr/bin/env python3
"""Benchmark discrete residual-refinement variants on W1-W4 using the improved Huber energy base."""
...
def _candidates() -> list[Candidate]:
    ...
    return [
        Candidate(
            name="discrete_residual_balanced",
            objective="Use a discrete residual action menu with tiny h5 and small long-horizon moves, preserving only a few bounded corrections.",
            expectation="Expected mean uplift vs improved base: roughly 0.0% to +0.5%. Should keep some W3 help while reducing W1/W4 overcorrection.",
            overrides=_merge(
                baseline,
                {
                    "llm": {
                    ...
                    "hdelta": {
                        "freeze_horizons": [1],
                        "max_adjustment_pct": 0.85,
                        "case_match_top_k": 2,
                        "case_min_examples": 2,
                        "case_min_sign_agreement": 0.67,
                        "case_bound_scale": 0.65,
                        ...
                        "discrete_activation_fraction_by_horizon": {
                            "h5": 0.18,
                            "h20": 0.14,
                            "h30": 0.14,
                        },
                        "discrete_min_confidence_by_horizon": {
                            "5": "medium",
                            "20": "medium",
                            "30": "high",
                        },
        ...
        Candidate(
            name="discrete_residual_long_only",
            objective="Freeze h5 and allow only discrete h20/h30 residual actions, forcing the refiner into a sparse long-horizon correction role.",
            expectation="Expected mean uplift vs improved base: roughly -0.2% to +0.4%. Lower variance than the balanced variant, but a real risk of giving up too much W3 upside.",
            overrides=_merge(
                baseline,
                {
                    "llm": {
...
def main() -> int:
    ...
    if args.out_dir:
        ...
        out_dir = (
            PROJECT_ROOT
            / "reports"
            / "uk_ets_discrete_refinement_w1_w4_huber_energy"
