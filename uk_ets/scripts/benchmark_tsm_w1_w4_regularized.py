#!/usr/bin/env python3
"""Regularization-focused TSM benchmark on repeated-holdout windows W1-W4."""
...
def _compact_features(max_features: int) -> dict[str, Any]:
    out = dict(LIVE_CFG["features"])
    out["max_exogenous_features_model"] = max_features
    out["preferred_feature_order"] = out["preferred_feature_order"][:max_features]
    return out


def _candidate_overrides() -> dict[str, dict[str, Any]]:
    return {
        "baseline_live_auto_uk": {
            "llm": {"methods": []},
        },
        "shared_lr1e3_e40_pat8": {
            "llm": {"methods": []},
            "model": {
                "learning_rate": 0.001,
                "max_epochs": 40,
...
def main() -> int:
    out_dir = (
        PROJECT_ROOT
        / "reports"
        / "uk_ets_tsm_w1_w4_regularized_benchmark"
