...
WINDOW_SUMMARIES = (
    ("W0", "Jul 2025 to Jan 2026", "2025-07-01", "2026-01-22"),
    ("W1", "Oct 2024 to May 2025", "2024-10-28", "2025-05-20"),
    ("W2", "Feb 2024 to Sep 2024", "2024-02-23", "2024-09-16"),
    ("W3", "Jun 2023 to Jan 2024", "2023-06-21", "2024-01-12"),
    ("W4", "Oct 2022 to May 2023", "2022-10-17", "2023-05-10"),
)
...
def _build_metrics_payload(
    ...
    *,
    ...
    for key, period, origin_start, origin_end in WINDOW_SUMMARIES:
        window_rows = realized[
            (realized["forecast_made_on"].astype(str) >= origin_start)
            & (realized["forecast_made_on"].astype(str) <= origin_end)
        ].copy()
        if window_rows.empty:
            continue

        window_complete_counts = window_rows.groupby("forecast_made_on")["step_index"].nunique().sort_index()
        window_complete_origins = (
            window_complete_counts[window_complete_counts == max_horizon].index.tolist() if max_horizon else []
        )
        window_complete_rows = window_rows[window_rows["forecast_made_on"].isin(window_complete_origins)].copy()
        base_window_path_summary = _mse_summary(window_complete_rows, "base_tsm_forecast")
        llm_window_path_summary = _mse_summary(window_complete_rows, "llm_tsm_forecast")

        horizon_20_rows = window_rows[window_rows["step_index"] == 20].copy()
        base_h20_summary = _mse_summary(horizon_20_rows, "base_tsm_forecast")
        llm_h20_summary = _mse_summary(horizon_20_rows, "llm_tsm_forecast")

        horizon_30_rows = window_rows[window_rows["step_index"] == 30].copy()
        base_h30_summary = _mse_summary(horizon_30_rows, "base_tsm_forecast")
        llm_h30_summary = _mse_summary(horizon_30_rows, "llm_tsm_forecast")

        if (
            base_window_path_summary is None
            or llm_window_path_summary is None
            or base_h20_summary is None
            or llm_h20_summary is None
            or base_h30_summary is None
            or llm_h30_summary is None
        ):
            continue

        base_path_mse = float(base_window_path_summary["mse"])
        llm_path_mse = float(llm_window_path_summary["mse"])
        base_h20_mse = float(base_h20_summary["mse"])
        llm_h20_mse = float(llm_h20_summary["mse"])
        base_h30_mse = float(base_h30_summary["mse"])
        llm_h30_mse = float(llm_h30_summary["mse"])

        window_summaries[key] = {
            "period": period,
            "origin_start": origin_start,
            "origin_end": origin_end,
            "settled_origin_count": int(window_rows["forecast_made_on"].nunique()),
            "complete_origin_count": int(len(window_complete_origins)),
            "settled_h20_count": int(len(horizon_20_rows)),
            "settled_h30_count": int(len(horizon_30_rows)),
            "base_tsm_path_mse": base_path_mse,
            "llm_tsm_path_mse": llm_path_mse,
            "base_tsm_h20_mse": base_h20_mse,
            "llm_tsm_h20_mse": llm_h20_mse,
            "base_tsm_h30_mse": base_h30_mse,
            "llm_tsm_h30_mse": llm_h30_mse,
            "path_uplift_pct": float(((base_path_mse - llm_path_mse) / base_path_mse) * 100.0) if abs(base_path_mse) > 1e-12 else 0.0,
            "h20_uplift_pct": float(((base_h20_mse - llm_h20_mse) / base_h20_mse) * 100.0) if abs(base_h20_mse) > 1e-12 else 0.0,
            "h30_uplift_pct": float(((base_h30_mse - llm_h30_mse) / base_h30_mse) * 100.0) if abs(base_h30_mse) > 1e-12 else 0.0,
        }
...
def export_public_site_files(
    ...
    *,
    ...
    export_generated_at = iso_utc_now()
    ...
    metrics_payload = _build_metrics_payload(
        ...
        latest_generated_at=export_generated_at,
        latest_observed_date=latest_observed_date,
        model_version=model_version,
    )
