...
Data comes from EEX emissions auctions (2017-2025).
...
def parse_auction_file(file_path: Union[str, Path]) -> pd.DataFrame:
    ...
    # Read all lines to find the header row (use latin-1 for Euro symbol)
    with open(file_path, 'r', encoding='latin-1') as f:
        lines = f.readlines()
    
    # Find the row containing "Date" as first meaningful column
    header_row = None
    ...
    # Read the file with the identified header
    df = pd.read_csv(file_path, skiprows=header_row, encoding='latin-1', 
...
def load_auction_data(data_dir: Union[str, Path]) -> pd.DataFrame:
    ...
    auction_files = sorted(auction_dir.glob("emission-spot-primary-market-auction-report-*.csv"))
